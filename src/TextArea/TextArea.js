...
class TextArea extends Component{
    ...
    onChange(e){
        ...
        this.setState({number:(maxlength - this.textArea.value.length)});
        change(name,value);
    }
    render(){
        ...
        return(
            <div style={{position:'relative'}}>
                <textarea placeholder={ placeholder } ref={(input)=>{ this.textArea = input }} className='form-control' onChange={ this.onChange } maxLength={ maxlength || 10000 } value={ value }>
                    { value }
                </textarea>
